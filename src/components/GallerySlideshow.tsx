...
const GallerySlideshow = () => {
  ...
  return <div className="relative">
          ...
          <Card className="group hover:shadow-glow transition-all duration-300 overflow-hidden bg-background/10 backdrop-blur-sm max-w-4xl w-full">
            <CardContent className="p-0">
              ...
              <div className="aspect-[16/10] relative overflow-hidden">
                <img src={currentItem.image} alt={currentItem.description} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
      ...
      <div className="flex justify-center items-center gap-4 mt-8">
        <Button variant="outline" size="icon" onClick={prevSlide} className="bg-background/80 backdrop-blur-sm hover:bg-background/90">
        ...
        <div className="flex gap-2">
          {Array.from({
          length: totalSlides
        }).map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all ${index === currentSlide ? 'bg-primary' : 'bg-background/50 hover:bg-background/70'}`} />)}
        </div>

        <Button variant="outline" size="icon" onClick={nextSlide} className="bg-background/80 backdrop-blur-sm hover:bg-background/90">
